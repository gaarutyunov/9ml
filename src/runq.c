...
void error_usage(void) {
    fprint(2, "Usage:   runq <checkpoint> [options]\n");
    fprint(2, "Example: runq model.bin -n 256 -i \"Once upon a time\"\n");
    fprint(2, "Options:\n");
...
main(int argc, char *argv[])
{
    ...
    for (int i = 2; i < argc; i+=2) {
        ...
        else if (argv[i][1] == 's') { rng_seed = atoll(argv[i + 1]); }
    ...
    if (rng_seed <= 0) rng_seed = (uvlong)time(0);
