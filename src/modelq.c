...
/* RMSNorm SIMD - defined in simd_amd64.s */
extern void rmsnorm_simd(float *o, float *x, float *weight, int size);

/* Quantized matmul SIMD - defined in simdq_amd64.s */
extern void matmul_q8_simd(float *xout, schar *xq, float *xs, schar *wq, float *ws, int n, int d, int gs);
#endif
...
/* RMSNorm dispatcher - uses SIMD when available */
...
/* Dispatch function - uses SIMD assembly when available */
...
#ifndef DISABLE_OPTIMIZATIONS
    if (opt_config.use_simd) {
        matmul_q8_simd(xout, x->q, x->s, w->q, w->s, n, d, GS);
    } else {
